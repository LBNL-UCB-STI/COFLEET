...
def moves_vmt_forecast(moves_definition_file, moves_vmt_growth_rate_file,
    ...
    hpms_vmt_growth_rate = read_csv(moves_vmt_growth_rate_file)
    # pop_turnover_rate = read_csv(MOVES_turnover_rate_file)
    
    age_distribution_forecast = read_csv(moves_age_projection_file)
    ...
    fleet_mix_com_only.to_csv(moves_future_vmt_by_stmy_file)         
    ...
    moves_vmt_by_st.to_csv(moves_future_vmt_check_file) 
    ...
    plt.ylim([1, 2.5])
    plt.savefig(os.path.join(path_to_plot, 'plot_forecast', 'LDV_growth_factor_moves.png'), dpi = 300,
                bbox_inches = 'tight')
    plt.show()
    ...
    plt.legend(fontsize = 12)
    plt.ylim([1, 2])
    plt.savefig(os.path.join(path_to_plot, 'plot_forecast', 'com_growth_factor_moves.png'), dpi = 300,
                bbox_inches = 'tight')
    plt.show()
     
    # generate VMT growth rate in VIUS format
    mhd_rate = \
    ...
    vmt_growth_rate_for_vius.to_csv(vius_vmt_growth_rate_file) 
    ...
    plt.legend(fontsize = 12)
    plt.savefig(os.path.join(path_to_plot, 'plot_forecast', 'com_VMT_growth_vius.png'), dpi = 300,
                bbox_inches = 'tight')
    plt.show()
