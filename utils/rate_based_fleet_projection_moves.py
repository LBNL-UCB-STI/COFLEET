...
def fleet_size_projection_moves(moves_definition_file, moves_vmt_growth_rate_file, 
    ...
    hpms_vmt_with_def.to_csv(moves_vmt_growth_rate_file, index = False)
    ...
    source_type_population_with_sale.to_csv(moves_turnover_rate_file, index = False)
    ...
    # if adjust_tail:
        # fleet_mix_by_year.to_csv(os.path.join(path_to_moves, 'turnover', 'age_distribution_moves_fixtail.csv'),
                                 # index = False)
    # else:
    fleet_mix_by_year.to_csv(moves_age_projection_file, index = False)
    ...
    if adjust_tail:
        plt.savefig(os.path.join(path_to_plot, 'plot_forecast', 'com_age_projection_moves_fixtail.png'), dpi = 300,
                    bbox_inches = 'tight')
    else:
        plt.savefig(os.path.join(path_to_plot, 'plot_forecast', 'com_age_projection_moves.png'), dpi = 300,
                    bbox_inches = 'tight')    
    plt.show()
