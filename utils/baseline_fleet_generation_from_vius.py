...
def vius_baseline_fleet_generator(vius_data_cleaned, moves_definition_file, 
                                  moves_vmt_by_stmy_file, vius_base_vmt_by_stmy_file,
                                  vius_rmar_file, vius_base_vmt_by_fuel_file, path_to_plot):
    
    """
   Generates baseline fleet distributions, age profiles, VMT fractions, and RMAR factors
   from cleaned VIUS survey data, aligning results with MOVES model definitions.
   ...
   """
    vius_fleet = read_csv(vius_data_cleaned)
    
    age_distribution = pd.read_excel(moves_definition_file, 
                                    sheet_name = 'AGE_distribution')
    
    moves_vmt_distribution = read_csv(moves_vmt_by_stmy_file)
    ...
    vius_age_distribution.to_csv(vius_base_vmt_by_stmy_file,
                                 index = False)
    ...
    vius_RMAR.to_csv(vius_rmar_file, index = False)
    # <codecell>
    ...
    vius_age_distribution_with_fuel.to_csv(vius_base_vmt_by_fuel_file, index = False)